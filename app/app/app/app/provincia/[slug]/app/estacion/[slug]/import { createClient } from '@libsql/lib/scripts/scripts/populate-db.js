...
for (const estacion of estaciones) {
  const slug = estacion.nombre.toLowerCase().replace(/\s/g, '-').replace(/[áéíóú]/g, match => ({
    'á': 'a', 'é': 'e', 'í': 'i', 'ó': 'o', 'ú': 'u'
  })[match] || match);
  
  await db.execute({
    sql: `INSERT OR IGNORE INTO estaciones (slug, nombre, ciudad, provincia, direccion, lat, lon, articulo)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
